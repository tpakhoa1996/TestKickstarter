...
//=================== SUCCESSFUL===============
	if (data.projectStatus != 'successful') {
		data.projectGoal = $("#pledged").attr('data-goal');
		data.projectPledged = $("#pledged").attr('data-pledged');
		tmp = $("span.project_currency_code");
...
// ==========================================
	tmp = $(".NS_projects__badges [href^='/discover/categories']");
	if (!tmp.first().attr('href'))
		tmp = $(".NS_projects__category_location [href^='/discover/categories']");
	tmp = tmp.first().attr('href');
	data.projectCategory = url.parse(tmp).pathname.split("/").slice(3);
	data.projectPoster = $(".project-image img").attr('src');
	if (!data.projectPoster)
